...
const AssignApprove = () => {
  ...
  useEffect(() => {
    ...
    async function getProjectDetails() {
      ...
      let todos = newtodo?.data?.projectByProjectApprovedID?.items;
      if (todos) {
        todos.map((todo) => {
          console.log(todo);
          setUserID(todo.username);
  ...
  const createTodo = async () => {
    try {
      await checkDuplicate(userId, creator.creatorId);
    } catch (error) {
      console.log("Error creating todo", error);
    }
  };

  const checkDuplicate = async (userID, creatorID) => {
    const checkDuplicateQuery = `
      query ListCreatorChats($userID: String!, $creatorID: String!) {
        listCreatorChats(filter: { userID: { eq: $userID }, creatorID: { eq: $creatorID } }) {
          items {
            id
          }
        }
    ...
    const checkDuplicateResult = await API.graphql(
      graphqlOperation(checkDuplicateQuery, { userID, creatorID })
    );
    if (checkDuplicateResult.data.listCreatorChats.items.length > 0) {
      console.log("Todo with the same email and phone already exists!");
      await assignCreator();
    } else {
  ...
  async function assignCreator() {
    ...
    nav(
      `/admin/${id}/panel/${pId}/details/a p r o v e/congrats/${creator.creatorName}/${creator.creatorId}/`
    );
  }
  ...
  async function createConnection() {
    ...
    const newtodo = await API.graphql(
      graphqlOperation(createCreatorChats, { input: todoDetails })
    );
    let todos = newtodo?.data?.createCreatorChats?.items;
    if (todos) {
      console.log("connection", todos);
    }
  }
  return (
    <>
            ...
            return (
              <Card>
                ...
                <Title>
                  ...
                  <Button onClick={createTodo}>ASSIGN</Button>
                </Title>
              </Card>
            );
          })}
